...
size_t LocalLinearPredictionStrategy2::prediction_length() const {
  return lambdas.size();
}

std::vector<double> LocalLinearPredictionStrategy2::predict(
    size_t sampleID,
  ...
  Eigen::VectorXd weights_vec = Eigen::VectorXd::Zero(num_nonzero_weights);
  {
    size_t i = 0;
  ...
  for (size_t i = 0; i < num_nonzero_weights; ++i) {
    for (size_t j = 0; j < num_variables; ++j){
      ...
      X(i, j) = train_data.get(indices[i], current_predictor) - data.get(sampleID, current_predictor);
    }
 ...
 *
 */
std::vector<double> LocalLinearPredictionStrategy2::compute_variance(
    size_t sampleID,
